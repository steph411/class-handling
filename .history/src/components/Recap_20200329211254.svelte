<script>
  ...
  $: console.log({ studentName, fail, initialNotes });
    ...
    $classesStore.map(element => ({
      ...
      value:
        (selectedStudent &&
          ...
          selectedStudent.notes.find(el => el.nom == element.nom).value) ||
        0,
      coefficient: element.coefficient
    }));
  $: average = selectedStudent && selectedStudent.moyenne;
  //   $: selectedStudentId ? (average = 0) : console.log("Resetting average");
